...
static ssize_t http_handle_announce( const int64 client_socket, char *data ) {
  ...
  while( scanon ) {
    ...
    /* xyt */
    case 3:
      if (!byte_diff(data, 3, "ono"))
      {
	len = scan_urlencoded_query( &c, data = c, SCAN_SEARCHPATH_VALUE );
	if ((len <= 0) || scan_ono_data(data, len, &ono)) HTTPERROR_400_PARAM;
#ifdef ONO_DEBUG
	printf("ono: %s\n", ono);
#endif
      }
  ...
  if( OT_FLAG( &peer ) & PEER_FLAG_STOPPED )
    ...
    if( !torrent || !( len = return_peers_for_torrent( hash, numwant, &peer, ono, SUCCESS_HTTP_HEADER_LENGTH + static_outbuf, 1 ) ) ) HTTPERROR_500; /* xyt */
  }
